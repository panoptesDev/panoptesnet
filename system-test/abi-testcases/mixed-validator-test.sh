...
bootstrapInstall() {
  ...
  if [[ ! -h $solanaInstallDataDir/active_release ]]; then
    sh "$PANOPTES_ROOT"/install/panoptes-install-init.sh "$v" "${solanaInstallGlobalOpts[@]}"
  fi
...
for v in "${otherVersions[@]}"; do
  panoptes-install-init "${solanaInstallGlobalOpts[@]}" "$v"
  panoptes -V
done
...
solanaInstallUse() {
  ...
  PANOPTES_BIN="$solanaInstallDataDir/releases/$version/solana-release/bin"
