...
use clap::ArgMatches;
use solana_clap_utils::{input_parsers::lamports_of_pano, offline::SIGN_ONLY_ARG};
use solana_client::rpc_client::RpcClient;
use solana_sdk::{
    commitment_config::CommitmentConfig, fee_calculator::FeeCalculator, message::Message,
    native_token::lamports_to_pano, pubkey::Pubkey,
};
...
impl SpendAmount {
    ...
    pub fn new_from_matches(matches: &ArgMatches<'_>, name: &str) -> Self {
        let amount = lamports_of_pano(matches, name);
        ...
        if from_pubkey == fee_pubkey {
            if from_balance == 0 || from_balance < spend + fee {
                return Err(CliError::InsufficientFundsForSpendAndFee(
                    lamports_to_pano(spend),
                    lamports_to_pano(fee),
                    *from_pubkey,
                ));
            }
        } else {
