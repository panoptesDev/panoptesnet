...
use solana_sdk::{
    ...
    native_token::lamports_to_pano, pubkey::Pubkey, system_instruction::SystemError,
        ...
        SpendAmount::Some(withdraw_amount) => {
            if current_balance.saturating_sub(withdraw_amount) < minimum_balance {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} PANO to remain rent exempt", lamports_to_pano(minimum_balance)
                ))
                .into());
            }
            withdraw_amount
