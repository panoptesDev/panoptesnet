...
stake_pano=1   # default number of PANO to assign as stake (1 PANO)
...
usage: $0 [OPTIONS] <PANO to stake ($stake_pano)>
...
if ((airdrops_enabled)); then
  ...
  $solana_cli \
    ...
    transfer --allow-unfunded-recipient "$keypair" "$stake_pano"
fi
...
$solana_cli "${common_args[@]}" \
  create-stake-account "$stake_account" "$stake_pano"
$solana_cli "${common_args[@]}" \
