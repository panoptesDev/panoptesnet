...
```bash
panoptes-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
panoptes-tokens transaction-log --output-path transactions.csv
```
...
```bash
panoptes-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
panoptes-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
panoptes-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
panoptes-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
panoptes-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `panoptes-tokens balances`.
...
```bash
panoptes-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
