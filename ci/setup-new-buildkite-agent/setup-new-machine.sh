...
cat >/etc/apt/apt.conf.d/99-panoptes <<'EOF'
...
"$PANOPTES_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$PANOPTES_ROOT"/net/scripts/install-certbot.sh
...
"$PANOPTES_ROOT"/net/scripts/install-earlyoom.sh
"$PANOPTES_ROOT"/net/scripts/install-nodejs.sh
"$PANOPTES_ROOT"/net/scripts/localtime.sh
"$PANOPTES_ROOT"/net/scripts/install-redis.sh
"$PANOPTES_ROOT"/net/scripts/install-rsync.sh
"$PANOPTES_ROOT"/net/scripts/install-libssl-compatability.sh
