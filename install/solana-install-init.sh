...
if [ -z "$PANOPTES_DOWNLOAD_ROOT" ]; then
    PANOPTES_DOWNLOAD_ROOT="https://github.com/solana-labs/solana/releases/download/"
fi
...
main() {
    ...
    *)
    ...
    # Check for PANOANA_RELEASE environment variable override.  Otherwise fetch
    # the latest release tag from github
    if [ -n "$PANOANA_RELEASE" ]; then
      release="$PANOANA_RELEASE"
    else
    ...
    download_url="$SOLANA_DOWNLOAD_ROOT/$release/panoptes-install-init-$TARGET"
    solana_install_init="$temp_dir/panoptes-install-init"
