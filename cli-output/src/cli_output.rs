            ...
            match &keyed_reward.reward {
                Some(reward) => {
                    writeln!(
                        ...
                        lamports_to_pano(reward.amount),
                        lamports_to_pano(reward.post_balance),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} PANO", lamports_to_pano(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} PANO", lamports_to_pano(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} PANO", lamports_to_pano(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} PANO", lamports_to_pano(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            for reward in rewards {
                ...
                writeln!(
                    ...
                    format!(
                        "{}◎{:<14.9}",
                        sign,
                        lamports_to_pano(reward.lamports.abs() as u64)
                    ),
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_pano(total_rewards.abs() as u64)
            )?;
        }
